...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # New:      32,  64, 128, 256,  512
        
        self.enc1 = nn.Sequential(CBR(in_channels, 32), CBR(32, 32))
        ...
        # Concatenated channels: e4 (256) + upconv4_out (256) = 512
        self.dec4 = nn.Sequential(CBR(512, 256), CBR(256, 256))
        
        self.upconv3 = nn.ConvTranspose2d(256, 128, kernel_size=2, stride=2)
        # Concatenated channels: e3 (128) + upconv3_out (128) = 256
        self.dec3 = nn.Sequential(CBR(256, 128), CBR(128, 128))
        
        self.upconv2 = nn.ConvTranspose2d(128, 64, kernel_size=2, stride=2)
        # Concatenated channels: e2 (64) + upconv2_out (64) = 128
        self.dec2 = nn.Sequential(CBR(128, 64), CBR(64, 64))
        
        self.upconv1 = nn.ConvTranspose2d(64, 32, kernel_size=2, stride=2)
        # Concatenated channels: e1 (32) + upconv1_out (32) = 64
...
class DnCNN(nn.Module):
    def __init__(self, in_channels, out_channels, num_layers=9, num_features=32): # Reduced defaults
...
class REDNet(nn.Module):
    def __init__(self, in_channels, out_channels, num_layers=3, num_features=32): # Reduced defaults
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            h = self.relu(h)
            if i < self.num_layers - 1: # Store outputs for skip connections, except for the last conv layer
                encoder_outputs.append(h)
        
        for i in range(self.num_layers):
            h = self.deconv_layers[i](h)
            if i < self.num_layers - 1: # Add skip connections, except for the last deconv layer
